...
| <a name="input_target_user"></a> [target_user](#input_target_user)                                                   | SSH user used to connect to the target_host                                                                                                                         | `string`                                                                  | "root"                                                                     |    no    |
| <a name="input_pre_install_script"></a> [pre_install_script](#input_pre_install_script) | a script to run before the installation. The script is executed using `source`, which means it runs in the same shell as `nixos-anywhere.sh`. This allows it to access the same variables (like `sshArgs`, `sshConnection`, `tempDir`, etc). | `string` | `null` | no |
| <a name="input_post_install_script"></a> [post_install_script](#input_post_install_script) | a script to run after the installation. The script is executed using `source`, which means it runs in the same shell as `nixos-anywhere.sh`. This allows it to access the same variables (like `sshArgs`, `sshConnection`, `tempDir`, etc). | `string` | `null` | no |
